...
/* Modified by Dinu Cristian Mircea, 2010-04-09
 *
 * + Added a mechanism for blinking a LED to signify that the
 *   bootloader is active, and also to cause the application to be
 *   automatically started after a number of seconds with no programming
 *   activity elapses
 */
...
#endif
#if DCM_EXTENSIONS_ENABLE
#if DCM_AUTOSTART_ENABLE
static uchar            halfSecondsLeft;
#endif
#endif
...
{
#if DCM_BLINK_LED_ENABLE
	DCM_BLINK_LED_PORT ^= (1 << DCM_BLINK_LED_BIT); // blink LED
#endif
#if DCM_AUTOSTART_ENABLE
	halfSecondsLeft--;
	if (!halfSecondsLeft) exitMainloop = 1;
#endif
}
#endif
...
#if DCM_EXTENSIONS_ENABLE
#if DCM_BLINK_LED_ENABLE
	DCM_BLINK_LED_PORT &= ~(1 << DCM_BLINK_LED_BIT); // Turn off LED
#endif
...
#if DCM_AUTOSTART_ENABLE
	halfSecondsLeft = 2*DCM_AUTOSTART_SECONDS;
#endif
...
int main(void)
{
...
#if DCM_EXTENSIONS_ENABLE
#if DCM_BLINK_LED_ENABLE
	DCM_BLINK_LED_DDR |= 1 << DCM_BLINK_LED_BIT; // setup LED output port
	DCM_BLINK_LED_PORT |= 1 << DCM_BLINK_LED_BIT; // turn on LED
#endif
	...
	// Initialize autostart/blink led timer
	TCCR1A = 0;
